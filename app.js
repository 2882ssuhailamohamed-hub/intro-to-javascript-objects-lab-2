...
const starterPokemon=pokemon.find(pockemon =>pokemon.starter === true);
...
game.pokemon.forEach(poke => {
  if (starterIDs.includes(poke.id)) {
...
game.catchPokemon = function(pokemonObj) {
 ...
 game.catchPokemon(game.pokemon[0]);
/*
Exercise 11
...
game.gymStatus = function () {
  ...
  // Iterate through all gyms and count completion status
...
game.catchPokemon = function(pokemonName) {
  const pokemonObj = this.pokemon.find(
    p => p.name.toLowerCase() === pokemonName.toLowerCase()
  );
  if (!pokemonObj) {
    return `The Pokémon "${pokemonName}" does not exist.`;
  }
   const pokeballItem = this.items.find(item => item.name === "pokeball");
if (!pokeballItem || pokeballItem.quantity <= 0) {
    console.log("Not enough pokeballs to catch this Pokémon!");
